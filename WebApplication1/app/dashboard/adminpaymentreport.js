(function () {
    'use strict';
    var controllerId = 'adminpaymentreport';
    ...
    function experthistory(common, $scope, $modal, $location, querydatacontext, userdatacontext) {
        ...
        {
            if ($scope.username != '' && $scope.frdate != '' && $scope.todate != '')
                $scope.FilteredTransactions = $scope.Transactions.filter(function (r) {
                    return r.UserName.indexOf($scope.username)>-1 && common.dateCheck($scope.frdate, $scope.todate, r.PmtDate)
                });
            else if ($scope.username != '')
                $scope.FilteredTransactions = $scope.Transactions.filter(function (r) {
                    return r.UserName.indexOf($scope.username)>-1 
                });
            else if ($scope.frdate != '' && $scope.todate != '')
                $scope.FilteredTransactions = $scope.Transactions.filter(function (r) {
        ...
        function getTransactions() {
            debugger;
            userdatacontext.GetPaymentTransactions(true, 0).success(function (result) {
                debugger;
